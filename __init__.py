...
if not skip_init:
    ...
    # Step 0: Apply H200 optimizations early
    try:
    ...
    # Step 1: Register sam3 model folder with ComfyUI
    try:
        import folder_paths
    ...
    # Step 3: Import fast mask nodes and merge them
    try:
        ...
        NODE_CLASS_MAPPINGS.update(FAST_MASK_MAPPINGS)
        NODE_DISPLAY_NAME_MAPPINGS.update(FAST_MASK_DISPLAY)
        print(f"[SAM3] [OK] Fast mask nodes added: {', '.join(FAST_MASK_MAPPINGS.keys())}")
    except Exception as e:
        error_msg = f"Failed to import fast mask nodes: {str(e)}"
        INIT_ERRORS.append(error_msg)
        print(f"[SAM3] [WARNING] {error_msg}")

    # Step 4: Import server to register API endpoints
    try:
